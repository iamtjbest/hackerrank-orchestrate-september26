...
python3 evaluation/diagnostics.py                 # amount_safe_to_pay diagnostic table
```
...
| `evaluation/diagnostics.py` | Diagnoses `amount_safe_to_pay` mismatches: constraining category/date, conservative-estimate convention substitution test, downstream-vs-independent breakdown for other fields. |
...
will not always match to the cent. See `evaluation/diagnostics.md` for a
full per-sample breakdown and `evaluation/evaluate.py`'s output for current
per-field accuracy.
